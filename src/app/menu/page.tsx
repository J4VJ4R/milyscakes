  ...
  const downloadQR = () => {
    ...
    if (originalCanvas) {
      ...
      if (ctx) {
        ...
        ctx.fillText("ESCANEA Y DESCARGA", width / 2, 30);

        ctx.font = "bold 13px Arial";
        ctx.fillText("NUESTRO MENU PDF", width / 2, 50);
        ...
        ctx.fillText("Mily's Cakes", width / 2, bottomStart);

        ctx.font = "bold 13px Arial";
        ctx.fillStyle = "#a855f7";
        ctx.fillText("Escanealo para descargar el menu", width / 2, bottomStart + 20);

        const image = newCanvas.toDataURL("image/png");
        const link = document.createElement("a");
        link.href = image;
        link.download = "milyscakes-qr-menu.png";
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 bg-gray-50 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 animate-fade-in-up">
        <h1 className="font-dancing-script text-5xl font-bold text-gray-800 text-center">
          Escanea y descarga el menu
        </h1>
        <p className="text-gray-600 text-center max-w-md">
          Comparte este codigo QR con tus clientes para que al escanearlo descarguen de inmediato el menu en PDF.
        </p>
        ...
        >
          <div className="bg-white p-2 rounded-xl">
            {url ? (
              ...
              <div className="w-64 h-64 rounded-lg bg-gray-100 animate-pulse" />
            )}
          </div>
        ...
        <a
          href={url || downloadPath}
          className="text-sm md:text-base text-mily-purple hover:text-mily-purple-dark font-semibold underline underline-offset-4 transition-colors text-center break-all"
        >
          {url || downloadPath}
        </a>
